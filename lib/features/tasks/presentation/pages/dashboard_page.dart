                ...
                Expanded(
                  child: BlocBuilder<AcademicBloc, AcademicState>(
                    builder: (context, academicState) {
                      if (academicState is AcademicLoading || academicState is AcademicInitial) {
                        return const Center(child: CircularProgressIndicator(color: Color(0xFF1D4ED8)));
                      }

                      if (academicState is AcademicEmpty) {
                        return Center(
                          child: Column(
                              ...
                              SizedBox(
                                ...
                                child: StButton(
                                  ...
                                  onPressed: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(builder: (_) => const CreateTermPage()),
                                    );
                                  },
                                ),
                              ),
                      ...
                      if (academicState is AcademicError) {
                        return Center(
                          child: Text('Error: ${academicState.message}', style: const TextStyle(color: Colors.red)),
                        );
                      }

                      if (academicState is AcademicLoaded) {
                        final subjects = academicState.term.subjects;
                                        ...
                                        return BlocBuilder<TaskBloc, TaskState>(
                                          builder: (context, taskState) {
                                            ...
                                            if (taskState is TaskLoaded) {
                                              final subjectTasks = taskState.tasks.where((t) => t.subjectId == subject.id.toString()).toList();
                                              
                                              if (subjectTasks.isNotEmpty) {
                                                final completedTasks = subjectTasks.where((t) => t.isCompleted).length;
                                                progressValue = completedTasks / subjectTasks.length;
